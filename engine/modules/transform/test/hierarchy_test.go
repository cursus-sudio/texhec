...
func TestHierarchy(t *testing.T) {
	...
	setup.Transform.Pos().SaveComponent(parent, transform.NewPos(5, 5, 5))
	setup.Transform.Parent().SaveComponent(child, transform.NewParent(transform.RelativePos))

	setup.Transform.Pos().SaveComponent(child, transform.NewPos(5, 5, 5))
	...
	setup.Transform.Pos().SaveComponent(child, transform.NewPos(10, 10, 10))
	...
	setup.Transform.Pos().SaveComponent(child, transform.NewPos(0, 0, 0))
	setup.expectAbsolutePos(child, transform.NewPos(10, 10, 10))
}
